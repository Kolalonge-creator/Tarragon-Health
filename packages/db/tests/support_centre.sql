-- Patient Support & Service Centre — core invariants: the ticket RLS
-- boundary (a patient can never rewrite routing/status fields directly),
-- the §24.5 state machine via advance_support_ticket_status(), first-
-- response/awaiting-patient wiring on comments, the technical escalation
-- ladder (§24.9), the clinical-escalation authority gate (§24.7/24.8,
-- Care Coordinator excluded), and the complaints governance workflow
-- (§24.14/24.15) including the governance_review authority gate.
--
...
begin
  ...
  values ('00000000-0000-0000-0000-000000000099', v_patient1, 'appointment', 'Cannot reschedule my visit', 'I tried to move my appointment and the button does nothing.')
  ...
  ---------------------------------------------------------------- 2. Patient cannot change status directly.
  perform set_config('request.jwt.claims', json_build_object('sub', v_patient1, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  begin
    update public.support_tickets set status = 'resolved' where id = v_ticket_id;
    insert into test_result values (2, 'patient blocked from changing ticket status', 'FAIL', 'update succeeded');
  exception when others then
    insert into test_result values (2, 'patient blocked from changing ticket status', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);
  ...
  if v_status = 'assigned' then
    insert into test_result values (3, 'assign_support_ticket moves New -> Assigned (control)', 'PASS', null);
  else
    insert into test_result values (3, 'assign_support_ticket moves New -> Assigned (control)', 'FAIL', v_status::text);
  end if;

  ---------------------------------------------------------------- 4. Internal comment hidden from patient; staff reply stamps first_response_at.
  perform set_config('request.jwt.claims', json_build_object('sub', v_coordinator, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  insert into public.support_ticket_comments (ticket_id, body, is_internal)
  values (v_ticket_id, 'Internal note: checking the booking service logs.', true);
  insert into public.support_ticket_comments (ticket_id, body, is_internal)
  ...
  perform set_config('role', 'postgres', true);
  if v_count = 1 then
    insert into test_result values (4, 'patient sees only the non-internal comment', 'PASS', null);
  else
    insert into test_result values (4, 'patient sees only the non-internal comment', 'FAIL', format('patient saw %s comments, expected 1', v_count));
  end if;
  ...
  if v_ticket.first_response_at is not null then
    insert into test_result values (5, 'first_response_at stamped on first non-internal staff reply', 'PASS', null);
  else
    insert into test_result values (5, 'first_response_at stamped on first non-internal staff reply', 'FAIL', null);
  end if;

  ---------------------------------------------------------------- 6. awaiting_patient -> patient reply reopens to in_progress.
  perform set_config('request.jwt.claims', json_build_object('sub', v_coordinator, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  ...
  if v_status = 'in_progress' then
    insert into test_result values (6, 'patient reply reopens awaiting_patient -> in_progress', 'PASS', null);
  else
    insert into test_result values (6, 'patient reply reopens awaiting_patient -> in_progress', 'FAIL', v_status::text);
  end if;

  ---------------------------------------------------------------- 7. resolving requires a note.
  perform set_config('request.jwt.claims', json_build_object('sub', v_coordinator, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  begin
    perform public.advance_support_ticket_status(v_ticket_id, 'resolved', null);
    insert into test_result values (7, 'resolving without a note is blocked', 'FAIL', 'succeeded with no note');
  exception when others then
    insert into test_result values (7, 'resolving without a note is blocked', 'PASS', sqlerrm);
  end;
  ...
  if v_status = 'resolved' and v_ticket.resolved_by is not null and v_ticket.resolved_at is not null then
    insert into test_result values (8, 'resolve with a note succeeds, attribution stamped (control)', 'PASS', null);
  else
    insert into test_result values (8, 'resolve with a note succeeds, attribution stamped (control)', 'FAIL', null);
  end if;

  ---------------------------------------------------------------- 9. satisfaction: patient can rate once, not twice.
  perform set_config('request.jwt.claims', json_build_object('sub', v_patient1, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  ...
  begin
    ...
    insert into test_result values (9, 'a second satisfaction rating is blocked', 'FAIL', 'second rating succeeded');
  exception when others then
    insert into test_result values (9, 'a second satisfaction rating is blocked', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);

  ---------------------------------------------------------------- 10. close, then status_history has a full audit trail.
  perform set_config('request.jwt.claims', json_build_object('sub', v_coordinator, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  ...
  if v_count >= 3 then
    insert into test_result values (10, 'status_history logged every transition', 'PASS', format('%s rows', v_count));
  else
    insert into test_result values (10, 'status_history logged every transition', 'FAIL', format('only %s rows', v_count));
  end if;

  ---------------------------------------------------------------- 11. technical escalation ladder: Tier 1 -> 2 -> 3, then blocked.
  ...
  begin
    perform public.bump_support_ticket_technical_tier(v_tech_ticket);
    insert into test_result values (11, 'technical tier caps at 3 (Engineering)', 'FAIL', 'a 4th bump succeeded');
  exception when others then
    insert into test_result values (11, 'technical tier caps at 3 (Engineering)', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);
  select technical_tier into v_count from public.support_tickets where id = v_tech_ticket;
  if v_count = 3 then
    insert into test_result values (12, 'ladder actually reached Tier 3 (control)', 'PASS', null);
  else
    insert into test_result values (12, 'ladder actually reached Tier 3 (control)', 'FAIL', v_count::text);
  end if;

  ---------------------------------------------------------------- 13. clinical escalation: Care Coordinator blocked, clinical tier allowed.
  perform set_config('request.jwt.claims', json_build_object('sub', v_coordinator, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  begin
    perform public.escalate_support_ticket_to_clinical(v_tech_ticket, 'This looks like a medication-related question, not a bug.');
    insert into test_result values (13, 'Care Coordinator blocked from clinical escalation', 'FAIL', 'succeeded');
  exception when others then
    insert into test_result values (13, 'Care Coordinator blocked from clinical escalation', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);
  ...
  if v_ticket.escalated_alert_id is not null then
    ...
    if v_count = 1 then
      insert into test_result values (14, 'clinical-tier staff escalates ticket, real clinician_alert raised (control)', 'PASS', null);
    else
      insert into test_result values (14, 'clinical-tier staff escalates ticket, real clinician_alert raised (control)', 'FAIL', 'no matching clinician_alert row');
    end if;
  else
    insert into test_result values (14, 'clinical-tier staff escalates ticket, real clinician_alert raised (control)', 'FAIL', 'escalated_alert_id still null');
  end if;

  ---------------------------------------------------------------- 15. complaints: patient files, cannot edit afterward.
  perform set_config('request.jwt.claims', json_build_object('sub', v_patient1, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  ...
  if v_edited_description <> 'edited' then
    insert into test_result values (15, 'patient cannot edit a filed complaint', 'PASS', 'RLS no-opped the update');
  else
    insert into test_result values (15, 'patient cannot edit a filed complaint', 'FAIL', 'the edit went through');
  end if;

  ---------------------------------------------------------------- 16. cannot skip straight to governance_review.
  perform set_config('request.jwt.claims', json_build_object('sub', v_admin, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  begin
    perform public.advance_complaint_status(v_complaint_id, 'governance_review', 'skipping ahead');
    insert into test_result values (16, 'cannot skip received -> governance_review', 'FAIL', 'succeeded');
  exception when others then
    insert into test_result values (16, 'cannot skip received -> governance_review', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);

  ---------------------------------------------------------------- 17. walk the pipeline; a non-director/admin is blocked from governance_review.
  perform set_config('request.jwt.claims', json_build_object('sub', v_coordinator, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  ...
  begin
    ...
    insert into test_result values (17, 'Care Coordinator blocked from governance_review sign-off', 'FAIL', 'succeeded');
  exception when others then
    insert into test_result values (17, 'Care Coordinator blocked from governance_review sign-off', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);
  ...
  if v_complaint.status = 'governance_review' and v_complaint.governance_reviewed_by is not null then
    insert into test_result values (18, 'Clinical Director completes governance review (control)', 'PASS', null);
  else
    insert into test_result values (18, 'Clinical Director completes governance review (control)', 'FAIL', null);
  end if;

  ---------------------------------------------------------------- 19. a governance_review complaint is terminal.
  perform set_config('request.jwt.claims', json_build_object('sub', v_director, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  begin
    update public.complaints set governance_note = 'edited after the fact' where id = v_complaint_id;
    insert into test_result values (19, 'a governance_review complaint is terminal', 'FAIL', 'edit succeeded');
  exception when others then
    insert into test_result values (19, 'a governance_review complaint is terminal', 'PASS', sqlerrm);
  end;
  perform set_config('role', 'postgres', true);

  ---------------------------------------------------------------- 20. §24.15: complaint -> formal clinical incident.
  perform set_config('request.jwt.claims', json_build_object('sub', v_patient1, 'role', 'authenticated')::text, true);
  perform set_config('role', 'authenticated', true);
  insert into public.complaints (organisation_id, patient_id, category, description)
  ...
  if v_complaint.incident_report_id is not null then
    ...
    if v_count = 1 then
      insert into test_result values (20, 'complaint escalates into a real clinical_incident_reports row', 'PASS', null);
    else
      insert into test_result values (20, 'complaint escalates into a real clinical_incident_reports row', 'FAIL', 'no matching incident row');
    end if;
  else
