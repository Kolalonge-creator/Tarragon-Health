...
-- last_confirmed_at / last_confirmed_by only in the non-prescriber branch, AFTER an
-- unconditional `if has_prescribing_authority then return new; end if;`. So a
-- prescriber (Senior Medical Officer+) confirming a refill wrote refill_date but was never
...
-- outright), a senior doctor covering a shift with no Medical Officer on duty could
...
--   2. Senior Medical Officer confirms a refill        -> stamped (THE FIX)
--   3. Senior Medical Officer changes the dose         -> NOT stamped (no false credit)
...
begin
  ...
  -- Indemnity is set because a contracted Senior Medical Officer (case 2)
  -- requires current cover before activation.
  ...
  select last_confirmed_by into v_by from public.medications where id = v_med;
  insert into test_result values (2, 'Senior Medical Officer confirms refill -> stamped (THE FIX)',
    case when v_by = v_staff_id then 'PASS' else 'FAIL' end,
    'last_confirmed_by=' || coalesce(v_by::text, 'null'));
  ...
  insert into test_result values (3, 'Senior Medical Officer changes dose -> NOT stamped',
