...
- **Chief Medical Officer / Clinical Director** — everything the tiers above do, plus assigning a case to another doctor, visibility into the whole team's caseload, and clinical-governance authority (protocol/config sign-off, indemnity-exemption scope). This is the single top rung — director/governance authority is intrinsic to reaching it, not a separate flag (see below).
...
**Case assignment (new 2026-08-31):** reassigning an escalation or clinician alert to a doctor OTHER than yourself requires `doctor_tier = 'chief_medical_officer'` — enforced by two BEFORE UPDATE triggers (`private.enforce_escalation_reassignment_authority`, `private.enforce_clinician_alert_reassignment_authority`), UI at `/clinician/escalations`'s worklist ("Assign to…") and `canAssignCases()` in `apps/web/src/lib/clinical/doctor-tier.ts`. Self-claim (any qualifying clinical tier) is unaffected. This closed a real gap: before this migration, RLS alone let any org staff member reassign any doctor's case.
