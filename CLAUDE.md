...
- **Founder decision 2026-08-29 — the wearable band is a confirmed exception: Tarragon will sell it once real device pairing is proven.** This narrows, not reverses, the 2026-08-02 decision above — that decision's "no hardware" stance still stands in full for the BP cuff, glucometer, and scale (`recommend_only` in `device_catalog`, no sale, no commission, indefinitely). The schema already anticipated exactly this split: `device_catalog.fulfillment_type` has carried a `tarragon_owned` value, reserved specifically for the band, since the table shipped 2026-08-25 (`20260825175835_device_catalog.sql`) — see that migration's and the 2026-08-26 affiliate-link-removal migration's own comments. Nothing is active yet: no wearable-band row has ever been seeded, because no real Yucheng/YCAviation vendor SDK, hardware, or price exists to seed truthfully (see the 2026-08-25 archive entry). **Do not seed a band row, build the vendor-SDK bridge, or add a purchase/checkout flow for the band yet** — a real vendor relationship and a real end-to-end pairing test (the same `active`/`clinically_reviewed` gate every `device_catalog` row already requires) have to exist first; a placeholder bridge or a fake SKU would be worse than waiting, per the 2026-08-25 archive entry's own reasoning. Once that test passes, the band becomes a `tarragon_owned` catalog row with its own purchase/checkout path (not yet designed) while the BP cuff/glucometer/scale rows stay `recommend_only` — this is a narrow, single-SKU carve-out, not a reopening of "Tarragon sells no hardware" generally.
