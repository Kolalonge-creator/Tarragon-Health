...
3. **Care Coordination** — lab network, pharmacy network, specialist referrals, hospital handoffs. **Corrected 2026-08-25 — lab tests (only) are partner-fulfilled again, via a real, considerably more sophisticated engine than the flat-commission model this used to be.** The 2026-08-03 self-arranged-fulfilment decision (patient takes a request to any lab, pays them directly) is reverted for labs: Synlab Nigeria is now a real, signed, nationwide lab partner. **This was built directly against the live database, outside git, before this session started** (migrations `screen_price_source_contracted` through `switch_on_synlab`, applied 2026-08-21 — see the archive entry for the full discovery) — a first attempt in this session to build a simpler restore was deleted and replaced with a migration (`20260825201803_reconcile_synlab_partner_billing_git_drift.sql`) that reconstructs the real live state into git instead, followed by the actual go-live change (`20260825202139_lab_partner_fulfilment_default_nationwide.sql`). Pricing is computed server-side per patient from a real contracted price list (`screen_types.price_kobo`), never trusted from the client; provider cost, transmission tracking, statement reconciliation, revenue-treatment accounting (`net_agent`, a liability not revenue, per founder decision — not yet accountant-reviewed), and a per-reason refund policy all exist and are real, not scaffolding. Pharmacy collection and specialist referrals are deliberately untouched and remain self-arranged/"YOU PAY THE LAB". **Standing concern, not resolved:** whoever/whatever built this live-only system was still actively iterating on it during this session (a real order and a live function rewrite both postdate this session's own start) — find out who/what that is before assuming git and production are in sync again. See `docs/CLAUDE_SPRINT_HISTORY_ARCHIVE.md`'s 2026-08-25 entry for the full story.
