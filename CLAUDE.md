...
**Explicitly Phase 2/3, not initial launch** (confirmed 2026-07-15 — do not build functional code for these without an explicit ask): full specialist-matching engine + 8-stage referral-status pipeline, and patient-initiated wellness testing catalogue.

**Authorized for full build, 2026-07-16** (explicit founder ask — see Current Sprint entry dated 2026-07-16): Employer/HMO risk-stratification dashboards (superseding the "early/placeholder builds only" note previously here for `/dashboard/corporate`/`/dashboard/hmo`), Premium ParentCare as a real subscription tier (superseding "today only a marketing page"), **home sample collection**, and **medication delivery logistics**. The latter two keep their patient-facing entry points auto-gated on real active logistics-partner data existing per region, rather than a separate feature flag — see Current Sprint for the exact mechanism.
...
### 2026-07-16 — Employer/HMO dashboards, Premium ParentCare, home logistics: explicit founder authorization to build fully
Founder gave direct, explicit, in-session authorization to build all three of the following fully (functional code, not schema scaffolding), overriding the 2026-07-15 Phase 2/3 deferral for these specific items — see the updated Clinical Tier Ladder and "What Claude Must Never Do" sections above:
1. **Employer/HMO risk-stratification dashboards** — real HMO dashboard (parity with the existing `/dashboard/corporate` cohort-analytics pipeline), a derived `patient_care_gaps` view (overdue screenings, stale chronic monitoring, unactioned abnormal results — no new mutable table, gaps are derived state), and a `cohort_cost_model_constants`-driven "modeled estimate" cost-avoided figure for HMO renewal conversations (must always carry a visible "estimate, not a real claims feed" disclaimer — never bare currency).
2. **Premium ParentCare as a real subscription tier** — new standalone plan codes (NGN + GBP + USD, monthly + yearly), up to 2 parents included with additional parents via a per-slot add-on (mirroring the existing `extra-family-member*` add-on pattern), `dedicated_coordinator` built into the base plan (not add-on-gated, unlike everywhere else it appears), and shared quarterly-PDF-report infrastructure (also closes a pre-existing gap where Family Premium already promised this and nothing implemented it).
3. **Home sample collection + medication delivery logistics** — built as fully functional backend/ops infrastructure (new `home_visit_providers`/`logistics_partners` global partner tables, FK/tracking columns on `lab_orders`/`pharmacy_orders`, new `home_visit`/`delivery` commission types), but the patient-facing entry point stays gated purely on real active partner data existing per region — zero active partners seeded today means patients see a static "coming soon in your area" block; the day ops adds a real active partner row, the real booking/tracking UI appears automatically, no redeploy, no separate feature flag.

Built on three independent branches (`claude/employer-hmo-risk-dashboards`, `claude/parentcare-subscription-tier`, `claude/home-collection-delivery-logistics`) since the three share no tables/RLS helpers beyond the existing `is_org_staff`/`is_admin`. See each branch's own commits for exact migration/file lists and verification detail.
...
- Never build functional code (not just schema scaffolding) for the full referral-matching pipeline or patient-initiated wellness testing without an explicit ask — see Clinical Tier Ladder above
- **Superseded 2026-07-16** — home sample collection, medication delivery logistics, Employer/HMO risk dashboards, and Premium ParentCare as a real subscription tier were explicitly authorized for full build this date (founder ask, direct in-session confirmation); this bullet previously grouped them with the still-deferred referral-matching pipeline/wellness-testing items above — see Current Sprint entry dated 2026-07-16 for what was built and how the two logistics features stay hidden from patients until a real partner exists
