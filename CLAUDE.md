...
Active Service: TypeScript — **active initiative (2026-07-11): marketing site + platform convergence.** Platform escalation UI merged to `main-dev` (PR #14). Marketing on `feat/marketing-site-scaffold`: homepage, 4 priority programmes, pricing (four-label), contact/leads, `/medication` + `/labs` (all uncommitted on this branch as of 2026-07-11). Same session: built the previously-missing **AbnormalResultHandler** Edge Function (`supabase/functions/abnormal-result-handler`) — trigger now fires it immediately via `net.http_post` (`20260711130000_abnormal_result_handler_trigger.sql`) to send the clinician WhatsApp alert, draft a care_plan/specialist_referral, and message the patient; the DB-level safety net (screening_upgrades + clinician_alerts + 4h SLA) was already solid and is unchanged. **Deployed and verified live** (2026-07-11) against the linked Supabase project (`koiplnmbgnqnbywhpjlf`, via MCP — this environment has no Docker/psql/CLI) with an isolated throwaway test org/patient/clinician (cleaned up after): trigger fired, `care_plans` draft created, function returned 200, and WhatsApp/SMS sends failed gracefully to `audit_log` (no `WHATSAPP_TOKEN`/`TERMII_API_KEY` secrets set yet — expected). Still needed before it can actually deliver: set those secrets, and submit the two new template names (`abnormal_result_clinician_alert`, `abnormal_result_patient_followup`) for Meta approval. Two unrelated repo-hygiene gaps surfaced during verification, not yet fixed: (1) local migration filenames/timestamps don't match the remote project's applied migration history (e.g. remote has an applied `reinstall_pg_net_in_extensions_schema` migration with no corresponding local file at all); (2) `audit_log.organisation_id` was `on delete set null` but the append-only trigger blocked that system UPDATE too — **fixed 2026-07-11**, now `on delete restrict` (`20260711140000_audit_log_organisation_restrict.sql`), same convention as every other non-null `organisation_id` column; audit_log rows and the organisations they reference are now permanent by design, never orphan-cleanable. **Abnormal-screening E2E test added and passing** (2026-07-11): `apps/web/e2e/abnormal-screening-escalation.e2e.test.ts`, run via `pnpm test:e2e` (opt-in, not part of `pnpm test` — needs `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` in the shell env; targets the live linked project directly since branching needs a Supabase Pro-plan upgrade this org doesn't have). Writing it surfaced two more pre-existing, unrelated bugs in the deployed `private.handle_new_user()` trigger (profile auto-provisioning on signup) — **not fixed, flagged for separate follow-up**: (a) `supabase.auth.admin.createUser({ phone })` strips the leading `+` before the trigger copies it into `profiles.phone`, which then violates `profiles_phone_e164` and 500s — real signups are app/web email/password (`apps/web/src/app/signup/`) and don't hit this — no WhatsApp/phone-OTP signup exists or is planned (see the 2026-07-11 WhatsApp policy change below) — but it would still break any admin-created phone-bearing account; (b) the trigger fires on auth.users' initial INSERT, before GoTrue's follow-up UPDATE attaches `app_metadata`, so `role`/`organisation_id` passed to `createUser` are silently never picked up — every programmatically-created user ends up a `patient` in a hardcoded default org; the deployed function also has a coalesce-to-default-org fallback (`00000000-0000-0000-0000-000000000001`) that exists live but isn't in any committed migration (more drift, same shape as gap (1) above). **WhatsApp policy change (2026-07-11):** per explicit user decision, WhatsApp/SMS is now follow-up/notification-only platform-wide — see Non-Negotiable Business Rules. A codebase audit at the time of the change found nothing to remediate: signup was already app/web-only (email/password, no phone-OTP flow ever built), and every existing WhatsApp call site (`send-pending-notifications`, `abnormal-result-handler`) was already fire-and-forget/non-blocking with an SMS fallback and audit-log-on-failure — no feature anywhere depended on a WhatsApp send succeeding. This is a documentation/policy lock-in, not a code fix. `docs/ARCHITECTURE.md` and `docs/FEATURE_SPEC.md` still contain older WhatsApp-parity language ("every patient action must work via WhatsApp") — not yet reconciled with this change. **Next:** merge marketing PR, staging deploy; reconcile `docs/ARCHITECTURE.md`/`docs/FEATURE_SPEC.md` WhatsApp language with the new policy; decide on the `handle_new_user` bugs and migration-drift gaps. See `docs/MARKETING_SITE_SPEC.md` §7, `docs/ARCHITECTURE.md` §7/§17 item 5.
