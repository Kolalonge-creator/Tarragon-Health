...
- Paystack "Pay with Transfer" (temporary per-transaction virtual account, no BVN) for one-off booking/voucher/sponsor charges — proposed, not yet built — endpoints, webhook changes, state transitions, test plan, and why Dedicated Virtual Accounts (Tier 2, needs a patient's BVN) stay explicit-ask-gated → `docs/PAYSTACK_PAY_WITH_TRANSFER_SPEC.md`
