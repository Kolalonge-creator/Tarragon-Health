...
- **WhatsApp/manual entry is never removed.** Device sync is an additive faster path into the same vitals tables patients already log into manually — same downstream escalation logic, same `patient_risk_scores`, same abnormal-result pipeline. No dual source-of-truth.
