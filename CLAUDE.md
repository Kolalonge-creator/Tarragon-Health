...
- Diaspora growth-pitch reconciliation (gift-a-health-check, standalone video consult, group screening days, instalment payment, screening→chronic conversion, referral commissions) against what's actually shipped → `docs/DIASPORA_HEALTH_CHECK_BUSINESS_MODEL_RECONCILIATION.md` — a reconciliation doc, not a build order. Resolved 2026-08-29: `screen_core`'s dead video-consult trigger branch is vestigial, not a broken promise — its real doctor-review mechanism is the async `annual_health_checks` review pipeline, already live; the gift flow sells the panel plus an optional standalone video-visit add-on rather than claiming a bundled call `screen_core` doesn't have.
