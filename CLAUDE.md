...
- Patient Identity & Master Patient Index — Tarragon patient identifier, identity verification tiers, duplicate detection/merge (unbuilt), wrong-patient/patient-switching prevention, proxy identities (parent/child, caregiver/elderly, sponsor clinical-access consent), identity audit trail, gap analysis against current code, phased Phase 1 (safe now) vs Phase 2 (needs explicit ask) recommendations → `docs/PATIENT_IDENTITY_MPI_SPEC.md` — a design/reconciliation doc; its §1 has the load-bearing distinction between the removed ParentCare *billing* tier (stays removed) and the live, separate `profile_access` proxy-identity system (not affected by that removal) — don't conflate the two
