...
| 4 | Instalment/thrift-style payment for the annual check | **Shipped**, and more general than proposed. `care_voucher_payments` layaway ("pay small small") lets a payer fund a voucher in any number of instalments of any size (above a configurable floor), not a fixed weekly plan. Recurring Paystack billing for the old subscription model is retained only as a legacy path for pre-existing subscribers (PR #418, 2026-08-31/09-02 pay-per-service cutover) — not a live parallel product; nobody can newly buy a subscription any more. Nothing to add. |
| 5 | Convert screening results into chronic care | **Shipped as infrastructure**, conversion funnel itself is a GTM/comms question, not code. The Category 2→1 escalation pipeline (abnormal result → clinician alert, 120min/24h two-tier SLA) is the platform's own highest-priority event per CLAUDE.md and already exists. `patient_risk_scores` gives infrastructure for a paid-plan incentive at result-reveal, but the screening-ladder subscriber discount was removed 2026-08-25 (margin was only 1.6%, not sustainable) and subscriptions themselves were retired 2026-08-31/09-02 — no discount or subscriber-tied incentive currently fires at result-reveal. What's not built: any deliberate upsell touchpoint at the result-reveal moment for the current pay-per-service model. Small, worth a follow-up ask if wanted — not attempted here. |
