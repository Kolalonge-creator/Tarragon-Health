...
2. **AI drafts, coordinator reviews and sends.** The AI-coach content already described above (education, adherence nudges, check-in prompts) can draft the reply text; the coordinator's job becomes reviewing and personalising a draft rather than composing from scratch. This still runs through Section 9's AI-oversight governance and stays inside the Care Coordinator's existing limits — never interpreting a result, never adjusting medication, never closing an escalation.
