...
- [ ] **Payments** — Paystack (NGN, recurring, webhooks: charge.success/charge.failure/subscription.disable, 7-day grace + dunning), Stripe (GBP diaspora, Customer Portal), invoice history, failed-payment handling, plan upgrade/downgrade, corporate billing. Pay with Transfer (temporary per-transaction NGN virtual account, no BVN) for one-off booking/voucher/sponsor charges: library code + lab-order UI built, unverified against live Paystack, other booking types/voucher/sponsor not yet wired — see `docs/PAYSTACK_PAY_WITH_TRANSFER_SPEC.md`
