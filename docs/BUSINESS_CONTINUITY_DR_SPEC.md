...
|---|---|---|---|
| 0 | Abnormal-result → Category-1 escalation pipeline | ≤ 1 hour | Tightest tier — the platform's own launch gate requires the clinician WhatsApp alert within 60 seconds, and the case then carries a two-tier clinical contact SLA — 120 minutes for a critical result, 1440 minutes (24h) for a non-critical abnormal result (`escalation_slas`, `screening_abnormal_result` pathway; see `CLAUDE.md`'s Non-Negotiable Business Rules). An hour of pipeline downtime already eats a meaningful slice of the 120-minute critical-result clock. |
...
| 2 | Payments (Paystack) | ≤ 24 hours | Genuinely loose — Paystack retries its own webhook delivery, and `supabase/functions/paystack-webhook/index.ts:145-172` already makes a delayed or duplicated replay a safe no-op via a `(provider, provider_event_id)` unique-constraint insert. (Stripe was removed entirely 2026-09-03 — no second provider to consider.) |
...
**Founder-blocked, this section:** Sentry uptime monitoring for these tiers exists but is only
partially live — per the last audit (`docs/MASTER_ARCHITECTURE_BLUEPRINT_GAP_ANALYSIS.md`, 2026-08-12,
not re-verified live since), 1 of 3 uptime monitors is active, 2 are disabled pending Sentry billing.
Re-check directly before assuming any of these RTO targets would actually page anyone.
...
| Paystack | New payments/service-purchase checkouts stall (subscriptions were retired 2026-09-02 — nothing recurring left to stall). | Webhook-driven with provider-side retry, plus local idempotency (`paystack-webhook/index.ts:145-172`, see Tier 2 above). |
