...
6. **Data residency + compliance first.** Supabase Postgres currently in **`eu-west-1`** (af-south-1 for NDPR was not selectable at creation — the NDPR target; revisit/migrate when available). Immutable audit log for every clinical, billing, and ML event.
...
| DB / Auth / Storage / Realtime | **Supabase Postgres**, `eu-west-1` (af-south-1 NDPR target when selectable), pgvector | RLS enforced platform-wide |
...
graph TB
  ...
  subgraph Data["Supabase (eu-west-1; af-south-1 NDPR target)"]
...
- **NDPR:** data residency target is `af-south-1` (currently on `eu-west-1` until af-south-1 is selectable — migrate when available); patient data export (JSON + PDF within 72h); right-to-erasure (anonymise personal fields, retain clinical minimum for the regulatory period).
...
| Postgres/Auth/Storage/Realtime | Supabase | **`eu-west-1`** now (`af-south-1` NDPR target when selectable) |
