...
**Corrected 2026-09-25, narrowed 2026-09-26 — speciality + years of experience live ONLY on a doctor's profile page, never inline in per-case attribution.** Founder decision 2026-09-25: Nigerian patients don't recognise an MDCN/NMCN credential number as a trust signal, so it should never be shown to a patient. The first implementation (2026-09-25) replaced the credential-number display with an inline "specialty · years of experience" line on every attribution touchpoint — corrected the next day, 2026-09-26: that inline detail was itself too much repetition. **Current model:** every patient-facing mention of a doctor — the shared `ReviewedByDoctor` component, `ReviewedResultLine`, and the async-consult/second-opinion/senior-case-review/patient-timeline/care-message attribution lines — renders **only "Dr. First Last"** via the shared `DoctorNameLink` component (`apps/web/src/components/doctor-name-link.tsx`), linked to a new dedicated profile page (`/doctor/[staffId]`, `apps/web/src/app/(dashboard)/doctor/[staffId]/page.tsx`, reachable from both patient and clinician views since it is not role-home-prefixed). That profile page is the *only* place `clinical_staff.specialty`, `years_of_experience`, and `bio` are shown to a patient. This does not reopen the "never promise ONE continuous named doctor" non-negotiable below — the profile page is reached because a specific doctor did something (reviewed a case, sent a message), framed as "about this doctor," never as a standing "your doctor" page. **Unchanged:** §5's licence-verification requirement is entirely internal/compliance — `credential_type`/`credential_number` stay on `clinical_staff`, still required and verified before activation, still gate `license_verified_at`; none of this touches whether a real MDCN/NMCN number is on file, only what a patient ever sees. Do not reintroduce a credential-number claim, or inline speciality/experience, into any patient-facing copy or component outside the profile page.
...
**Naming note (added 2026-08-31):** this section's "Tier 1/2/3" is an *attribution/branding* model — who gets named on which touchpoint — and is orthogonal to `clinical_staff.doctor_tier`, the DB seniority/authority ladder (also 3 real tiers as of 2026-08-31: Medical Officer, Senior Medical Officer, Chief Medical Officer, plus the non-clinical Care Coordinator floor — see CLAUDE.md's Clinical Tier Ladder section). The two numbering schemes do not correspond 1:1: this doc's "Tier 1 Clinical Director" maps onto `doctor_tier = 'chief_medical_officer'`; "Tier 3 Escalation Doctor" maps onto *whichever* `doctor_tier` clears `private.can_handle_emergency_escalation` (Senior Medical Officer or Chief Medical Officer) on a given case, not a fixed enum value. Don't conflate the two vocabularies when reading either doc.
...
| Patient/family dashboard | Persistent small badge, not a rotating claim | "Care protocols supervised by Dr. [Name]" — static, same for every patient | Pulled from `clinical_staff` where `doctor_tier = 'chief_medical_officer'` |
