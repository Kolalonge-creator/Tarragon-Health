...
/**
 * Where each trigger-activated CheckoutKind's own dedicated AFTER INSERT
 * trigger (see the charge.success handler's comment on this branch) actually
 * lands, and what a genuinely completed row looks like — read from each
 * trigger's own live definition, not guessed:
 ...
 * payment_provider_ref on success, so those two must match either column
 * depending on whether the trigger has already run by the time this code
 * checks (it always has, in practice, since it's a synchronous AFTER INSERT
 * on the very row this switch is processing — but matching only the
 * pre-trigger column would make this check permanently unable to find a
...
> = {
  voucher_payment: {
    ...
    refFilter: "pending_provider_ref.eq.%ref%",
    successStatus: "applied",
  },
  sponsored_subscription: {
    table: "service_purchases",
    refFilter: "payment_provider_ref.eq.%ref%",
...
// Split out of index.ts (which now only calls Deno.serve, unconditionally
// and unchanged — see its own header) so a test can import this module and
// call handleWebhookRequest directly, with a fake/local SupabaseClient and a
// constructed Request, without ever importing anything that binds a real
// port as a side effect. An earlier version of this split used an
// `if (import.meta.main)` guard around a shared Deno.serve call instead —
// reverted 2026-09-23 because whether Supabase's Edge Runtime loads a
// function's entrypoint in a way that makes `import.meta.main` true was
// genuinely unverified from this sandbox (no deploy credentials to smoke-
// test against a live/staging project), and getting it wrong would have
// silently killed the whole webhook in production — no listener, every
// Paystack call times out. This structural split removes that class of risk
// entirely: index.ts's own runtime behaviour is now byte-for-byte what it
// was before any of this file's test coverage existed.
    ...
    switch (event.event) {
      case "charge.success": {
        ...
        if (metadata.kind === "booking") {
          ...
          // correlation key or an unexpected error would have left the real
          // row un-activated while this code still reported success. That
          // is a SILENT FALSE SUCCESS, strictly worse for money-handling
          // code than the loud false FAILURE this branch was written to
          // fix. Mirroring the service_purchase branch above instead: query
          // each kind's own trigger-target table/status (see
          // TRIGGER_ACTIVATED_KIND_TABLE's header for where each of those
          // was read from) and only report processed if the trigger
          // genuinely got there.
          const config = TRIGGER_ACTIVATED_KIND_TABLE[metadata.kind];
          const { data: triggerRow } = await supabase
            .from(config.table)
            .select("id, organisation_id, status")
            .or(config.refFilter.replaceAll("%ref%", event.data.reference))
            .maybeSingle();

          if (!triggerRow) {
            await markFailed(`no ${config.table} row for reference=${event.data.reference}`);
          } else if (triggerRow.status !== config.successStatus) {
            await markFailed(
              `${config.table} ${triggerRow.id} still ${triggerRow.status} after trigger — see payment_integrity_flags`,
            );
          } else {
          ...
          // old catch-all `else` treated anything unrecognised as an
          // add_on, with no compiler or runtime signal that a kind had gone
          // unhandled. This is deliberately NOT full protection — a payload
          // bug (not a new enum member) can still put an unexpected string
          // in metadata.kind at runtime, past what the type system can see
          // — but that case now gets an honest, explicit "unrecognised
          // kind" failure instead of a silent, wrong subscription_add_ons
          // lookup.
          const unexpectedKind: never = metadata.kind;
          await markFailed(`charge.success with unrecognised metadata.kind=${String(unexpectedKind)}`);
        }
        break;
      }
