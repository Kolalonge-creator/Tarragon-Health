...
-- doctor_tier, employment_type, offers_therapy_sessions, years_of_experience)
-- for the same kind of clinician-profile display use case. The view carries
-- its own org/staff-scoping predicate because an owner-run view bypasses the
-- base table's RLS entirely — see reference_view_over_rls_needs_security_invoker.
