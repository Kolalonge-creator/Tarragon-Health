...
-- RLS
--
-- Reproductive health is a SPECIALLY-PROTECTED access category on this
-- platform, and these policies must route through it rather than through a
-- bare "some caregiver relationship exists" check.
--
-- The first version of this migration copied
-- 20260724001210_reproductive_health_profile.sql's caregiver shape verbatim
-- (`exists (profile_access where grantee_user_id = auth.uid())`). That shape
-- was already obsolete: 20260830103251 made reproductive_health a category
-- requiring its own explicit profile_access_categories grant, never unlocked
-- by a blanket relationship check and never by the dependent-account bypass
-- (private.can_read_clinical excludes reproductive_health from that branch
-- on purpose) or by cross-org break-glass (private.has_emergency_access
-- excludes it too). Copying the older table's comment -- "identical shape to
-- reproductive_health_profiles, deliberately" -- was exactly wrong, because
-- that table had already moved on.
--
-- Left as written, any caregiver holding ANY profile_access grant, even a
-- view-only one for medications, could have read a patient's full period,
-- symptom and mood history, and any 'manage' grantee could have written to
-- it. For a teenager on a parent-managed dependent account that is precisely
-- the disclosure the category was created to prevent.
--
-- SELECT goes through can_read_clinical/has_emergency_access for the
-- category. Writes require BOTH 'manage' AND an explicit reproductive_health
-- category grant -- deliberately not can_read_clinical alone, which is a
-- read gate a view-level grant satisfies.
--
-- Org staff read within the org (a care team needs this when a bleeding
-- pattern is the reason for the consultation) but never write: a cycle log
-- is the patient's own account of her body.
-- ---------------------------------------------------------------------------
