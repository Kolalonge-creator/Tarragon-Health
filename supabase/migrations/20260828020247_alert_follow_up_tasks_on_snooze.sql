...
-- No insert policy for authenticated: only the snooze trigger (SECURITY
-- DEFINER, below) creates these -- same "system populates, staff only
-- resolves" posture as care_outreach_tasks' own resolution stamping.
...
-- ---------------------------------------------------------------------------
-- Wire the snooze -> task creation into the existing lifecycle trigger
-- (part 2b). Re-stating the full function body (create or replace), same
-- convention this codebase already uses for layering behaviour onto a
-- shared trigger function across migrations (e.g.
-- private.handle_abnormal_screening_result was redefined by both
-- severity_driven_alert_urgency and abnormal_result_handler_trigger).
-- ---------------------------------------------------------------------------
