-- Patient Communication Architecture (77.13/77.15) — read-state tracking.
--
-- care_message_threads has never tracked whether either side has actually
-- opened a conversation (confirmed absent across every prior care_messages
-- migration). Without it, 77.13's "sent -> not opened -> reminder ->
-- escalation" pipeline has no signal to key off, and 77.15's audit trail has
-- no read-status column to report. This adds two clocks — one per side of
-- the conversation, not per message, matching the thread-level granularity
-- WhatsApp/most messaging UIs use ("you've read up to here") rather than a
-- row per message per reader, which nothing here needs — plus a denormalised
-- `last_message_author_role` so a scan for "care team hasn't read the
-- patient's latest message" is a single indexed comparison, not a subquery
-- into care_messages per thread.
--
-- A sponsor's read counts toward `patient_last_read_at`, the same side of
-- the conversation as the patient — 20260731181318 already treats sponsor
-- and patient as one "room" for delivery purposes (consent-gated via
-- profile_access.clinical_access), and there is no product reason to track
-- a third clock for a seat that already shares the patient's notification
-- fan-out.
...
comment on column public.care_message_threads.care_team_last_read_at is
  '77.13/77.15 read tracking. Stamped by mark_care_message_thread_read() when any org-staff caller opens the thread. Compared against last_message_at by the unread-message alert sweep (20260830014653_care_message_unread_escalation.sql) to detect a clinical-category patient message nobody on the care team has opened yet.';
...
comment on column public.care_message_threads.unread_alert_id is
  'The clinician_alerts row raised for the CURRENT unread period on this thread, if any (private.raise_unread_clinical_message_alerts, 20260830014653_care_message_unread_escalation.sql). Reset to null on every new care_messages insert so a later unread period can raise its own alert. Not itself read/write from the client.';
...
-- ---------------------------------------------------------------------------
-- Server-derived read-stamp RPC. Same auth shape as enforce_care_message_
-- author (is_org_staff / patient / consented sponsor) — a caller who is none
-- of those gets 'not authorised', matching that trigger's own errcode.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- after_care_message_insert — full replacement, carrying forward every
-- existing branch from the live 20260827203614 definition unchanged, plus
-- the new last_message_author_role/unread_alert_id bookkeeping.
-- ---------------------------------------------------------------------------
