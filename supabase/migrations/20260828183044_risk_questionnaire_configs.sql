-- Tarragon Health — Risk & Prevention Engine enhancement, 3/7
-- Configurable risk questionnaire + scoring engine (spec §2.4/§2.5/§2.18).
--
-- Mirrors 20260720122000_cv_risk_config.sql's proven shape exactly (a
-- versioned, signable jsonb config table + a forge-proof sign_*() RPC) —
-- the same pattern this codebase has already used twice for "clinical
-- parameters that need admin authorship + Director sign-off before they
-- can affect a real patient" (CV-risk targets, and via
-- 20260810033858_preventive_programmes_protocol_governance.sql's
-- protocol_slug gate, preventive-programme activation). A generic
-- normalized question/rule schema would be a THIRD, novel shape for the
-- same problem this codebase has already solved twice; reusing the proven
-- one is the better fit, not a shortcut.
--
-- The config jsonb holds the whole questionnaire (question bank with
-- per-question applicability predicates — spec's "branching logic reduces
-- unnecessary questions" — see apps/web/src/lib/rules/predicate.ts) AND
-- the per-condition scoring rules (factor points + thresholds), read by
-- apps/web/src/lib/rules/risk-questionnaire-engine.ts.
--
-- The seeded v1 row is a byte-for-byte port of the already-live hardcoded
-- engine (lib/rules/risk-scoring.ts's CONDITION_RULES + the fixed Zod form
-- in lib/validation/risk-assessment.ts) — proven equivalent by
-- risk-questionnaire-engine.parity.test.ts, which runs both engines
-- against the same scenarios risk-scoring.test.ts already covers and
-- asserts identical tiers. It ships UNSIGNED (approved_by/approved_at
-- null, is_active false) — same as cv_risk_config's seed — so today's
-- production behaviour does not change: the app keeps using the legacy
-- hardcoded engine until a real Clinical Director reviews and signs this
-- config through the new admin UI, at which point it takes over with
-- provably identical scoring. This is a deliberate, structural choice, not
-- an oversight: a health platform's risk engine should never start
-- computing patient risk from a different code path than yesterday
-- without a clinician having actually looked at it.
