...
-- Includes can_act_for so a caregiver can read back what they just screened
...
-- Includes can_act_for so a caregiver can read back their own request (also
-- required for INSERT ... RETURNING to work for them at all: Postgres checks
-- the new row against the SELECT policy for RETURNING to succeed).
