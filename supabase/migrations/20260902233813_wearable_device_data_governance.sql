...
--   - retention + access + deletion: genuinely new. device_data_retention_policies documents the retention
--     posture per data category (patient/staff-readable, admin-authored); device_data_deletion_requests +
...
--
-- NAMING (reconciled against main-dev at merge time): a same-day, independently-built §87.11
-- governance workflow also created public.data_deletion_requests and public.data_retention_policies
-- — a broader, platform-wide, human-reviewed DSAR workflow (any category, no auto-execution) that is
-- a genuinely different design from this migration's narrower, auto-executed, device/wearable-only
-- slice. Both are real and both ship; this migration's tables/types/RPC use a device_ prefix
-- (device_data_deletion_requests, device_data_retention_policies, device_data_deletion_scope,
-- device_data_deletion_status) so the two coexist under distinct names rather than colliding on
-- `create table`. Unifying them into one workflow is a real follow-up but out of scope for a merge
-- reconciliation.
...
-- ---------------------------------------------------------------------------
create table public.device_data_retention_policies (
...
comment on table public.device_data_retention_policies is
...
create unique index device_data_retention_policies_one_current on public.device_data_retention_policies (data_category) where is_current;

alter table public.device_data_retention_policies enable row level security;

create policy device_data_retention_policies_select on public.device_data_retention_policies
  for select to authenticated
  using (true);

create policy device_data_retention_policies_write on public.device_data_retention_policies
  for all to authenticated
  using (private.is_admin())
  with check (private.is_admin());

grant select, insert, update, delete on public.device_data_retention_policies to authenticated;

insert into public.device_data_retention_policies (data_category, retention_period, deletion_mechanism, is_current)
values
  ('wearable_readings', 'Retained for the lifetime of the connection plus 24 months, matching the platform''s general clinical-data retention posture.', 'Deleted on patient request via a device_data_deletion_requests row scoped to wearable_readings/device_connections/all_device_data, processed by org staff through public.execute_wearable_data_deletion(); otherwise retained.', true),
...
-- ---------------------------------------------------------------------------
create type public.device_data_deletion_scope as enum ('wearable_readings', 'device_connections', 'all_device_data');
create type public.device_data_deletion_status as enum ('requested', 'in_progress', 'completed', 'rejected');

create table public.device_data_deletion_requests (
  id                uuid primary key default gen_random_uuid(),
  organisation_id   uuid not null references public.organisations (id) on delete restrict,
  patient_id        uuid not null references public.profiles (id) on delete cascade,
  scope             public.device_data_deletion_scope not null,
  status            public.device_data_deletion_status not null default 'requested',
  reason            text,
...
comment on table public.device_data_deletion_requests is
...
create index device_data_deletion_requests_patient_idx on public.device_data_deletion_requests (patient_id, created_at desc);
create index device_data_deletion_requests_org_status_idx on public.device_data_deletion_requests (organisation_id, status);

alter table public.device_data_deletion_requests enable row level security;

create policy device_data_deletion_requests_select on public.device_data_deletion_requests
  for select to authenticated
  using (patient_id = (select auth.uid()) or private.is_org_staff(organisation_id));

create policy device_data_deletion_requests_insert on public.device_data_deletion_requests
...
create policy device_data_deletion_requests_staff_update on public.device_data_deletion_requests
  for update to authenticated
  using (private.is_org_staff(organisation_id))
  with check (private.is_org_staff(organisation_id));

grant select, insert on public.device_data_deletion_requests to authenticated;
grant update on public.device_data_deletion_requests to authenticated;
...
declare
  v_org      uuid;
  v_patient  uuid;
  v_scope    public.device_data_deletion_scope;
  v_status   public.device_data_deletion_status;
...
begin
  ...
  from public.device_data_deletion_requests
  ...
  update public.device_data_deletion_requests set status = 'in_progress' where id = p_request_id;
  ...
  if v_scope in ('device_connections', 'all_device_data') then
    with touched as (
      ...
      set status = 'disconnected', access_token = null, refresh_token = null, token_expires_at = null,
          ...
          revocation_reason = 'Deleted per device_data_deletion_requests ' || p_request_id::text
  ...
  if v_scope = 'all_device_data' then
    with touched as (
      ...
      set status = 'unpaired', unpaired_reason = 'Deleted per device_data_deletion_requests ' || p_request_id::text
  ...
  update public.device_data_deletion_requests
  set status = 'completed', processed_by = (select auth.uid()), processed_at = now()
  where id = p_request_id;

  insert into public.audit_log (organisation_id, actor_id, action, entity_type, entity_id, event)
  values (
    v_org, (select auth.uid()), 'device_data_deletion_requests.completed', 'device_data_deletion_requests', p_request_id,
    jsonb_build_object(
      'scope', v_scope,
...
begin
  ...
  if (select count(*) from public.device_data_retention_policies where is_current) <> 3 then
    raise exception 'expected 3 current device_data_retention_policies rows';
  end if;
  if not exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'device_data_deletion_requests') then
    raise exception 'device_data_deletion_requests was not created';
  end if;
