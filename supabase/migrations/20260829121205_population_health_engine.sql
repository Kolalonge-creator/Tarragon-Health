...
create table public.population_definitions (
  ...
  -- A system registry can be edited (an org may want a tighter CKD
  -- threshold than the platform default) but never deleted — see the
  -- delete policy below. Set only by the seed trigger, never by staff
  -- (enforced in the insert policy's with check).
  is_system         boolean not null default false,
  status            public.population_status not null default 'active',
  -- Typed filter vocabulary (all keys optional; an absent/empty key means
  -- "no constraint on this axis" — the intersection of every axis actually
  -- present is the population):
  --   conditions            text[]  care_plan_condition values — has an
  --                                 active care_plans row for any of these
  --   prevention_conditions text[]  prevention_condition values — has a
  --                                 prevention_risk_scores row for any of
  --                                 these (any tier, including 'unknown')
  --   risk_levels           text[]  risk_level values — latest tier for any
  --                                 scored prevention_condition is one of
  --                                 these
  --   care_gap_types        text[]  patient_care_gaps.gap_type values —
  --                                 currently has an open gap of this shape
  --   control_status        text[]  'controlled'|'uncontrolled'|'unknown' —
  --                                 see get_population_members() for the
  --                                 derivation (worst latest prevention risk
  --                                 tier among the patient's scored
  --                                 conditions; 'unknown' when never scored)
  --   engagement            text[]  'active'|'declining'|'disengaged' — from
  --                                 private.patient_engagement_events(),
  --                                 the real-behaviour signal (not
  --                                 pageviews) 2026-08-20 fixed the
  --                                 Analytics console onto
  --   min_age / max_age     integer
  --   sex                   text    'male' | 'female'
  --   states                text[]  profiles.state values
  --   pregnant_only         boolean profiles.is_pregnant = true
...
comment on column public.population_definitions.filters is
  'Typed segmentation filter — see the create table comment above for the full key vocabulary (spec §41.3). Every key is optional; an absent/empty array means unconstrained on that axis.';
...
-- A system registry stays forever — org staff can retire it (status =
-- 'archived') but not delete the row, so "who deleted the Diabetes
-- registry" is never a question anyone has to answer.
...
begin
  ...
  scored as (
    select
      ...
      -- "unknown" sits between high and moderate: an unscoreable patient is
      -- treated as more concerning than a confidently-low/moderate one, not
      -- less.
