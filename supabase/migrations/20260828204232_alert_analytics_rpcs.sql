-- Tarragon Health — Alert System infrastructure, analytics (8.13, and 8.7's
-- "measure alert burden").
--
-- Extends, rather than duplicates, the existing analyst-gated RPC family in
-- 20260717193112_analytics_console_phase2_rpcs.sql (same `if not
-- private.is_analyst() then return '{}'::jsonb end if` gate, same
-- SECURITY DEFINER + revoke-from-anon/grant-to-authenticated posture).
-- public.analytics_escalation_quality() already covers the abnormal-result
-- funnel, SLA met/breached and avg ack/resolution time for THAT pathway;
-- these two new RPCs cover the alert system as a whole (every category/type,
-- not abnormal-results-only) plus the metrics nothing existing covers at
-- all: escalation rate, false-positive rate, duplicate rate, and burden per
-- clinician.
