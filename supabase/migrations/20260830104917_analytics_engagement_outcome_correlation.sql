-- Tarragon Health — platform-wide engagement x outcome correlation
-- (Engagement/Retention gap #5). Same gate/style as its sibling
-- analytics_engagement_summary()/analytics_retention_cohorts() functions:
-- private.is_analyst()-gated, returns an empty result rather than erroring
-- for a non-analyst caller. Depends on patient_engagement_scores
-- (Engagement/Retention gap #2), so this migration necessarily follows it.
...
grant execute on function public.analytics_engagement_outcome_correlation() to authenticated;
