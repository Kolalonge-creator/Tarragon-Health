-- Caregiver Proxy Access, part 3: spend the granular vocabulary at the write
-- paths a caregiver actually uses.
--
-- Every one of these functions inlined its own `exists (select 1 from
-- profile_access where ... permission_level = 'manage')` check, most written
-- before private.can_act_for existed (2026-08-01) and never migrated onto
-- it. Redefined here to call private.can_act_for(beneficiary, permission)
-- instead — each function's own logic is otherwise byte-identical to its
-- current live definition (sponsor_book_care from the self-arranged sweep,
-- 20260803134416; the rest unchanged since they were written). A grant whose
-- permissions array is NULL (every grant that existed before this feature,
-- and every unscoped grant made after it) authorises all five exactly as
-- before — this is a narrowing, never a widening.
--
-- Mapping used throughout, matching the nine capabilities in
-- 20260829001500_caregiver_granular_permissions_schema.sql:
--   sponsor_book_care        book_appointments   (requests a self-arranged check)
--   sponsor_request_refill   manage_pharmacy
--   sponsor_pay_booking_order,
--   sponsor_payable_orders,
--   redeem_care_voucher      manage_payments
--   start_care_thread (+ the care_messages/care_message_threads RLS the
--   sponsor branch of that function relies on) communicate_with_care_team
--
-- Left on the coarse 'manage' gate deliberately, out of scope for this pass:
-- sponsor_set_dependent_basics (account setup, not one of the nine named
-- capabilities), purchase_care_voucher (already unconditionally refuses —
-- see 20260803134416), set_lab_order_facility (partner-only, dormant path),
-- and the sponsored-subscription webhook re-check in
-- 20260801092000_sponsor_notifications_and_sponsored_plans.sql (a payment
-- re-verification at webhook time, not a caregiver-initiated action — left
-- alone to avoid touching a live payment path untested in this pass).

-- --- 1. sponsor_book_care: book_appointments -----------------------------------
...
-- --- 2. sponsor_request_refill: manage_pharmacy --------------------------------
...
-- --- 3. sponsor_pay_booking_order: manage_payments ------------------------------
...
-- --- 4. sponsor_payable_orders: manage_payments ---------------------------------
-- Seeing what is owed is part of managing payments, not a separate read
-- capability — a sponsor who cannot pay has no use for a list of bills.
...
-- --- 5. redeem_care_voucher: manage_payments (third-party branch only) ---------
-- The self-spend branch (v_v.beneficiary_profile_id = v_caller) is untouched
-- — a patient redeeming their own voucher was never gated by profile_access
-- at all and stays that way.
...
-- --- 6. Messaging: communicate_with_care_team -----------------------------------
-- start_care_thread's sponsor branch, and the care_messages/care_message_threads
-- policies it relies on, move from the 1-arg can_read_clinical (any consented
...
begin
  ...
  -- Every redefined RPC must now route its authorisation through
  -- private.can_act_for/can_read_clinical rather than an inline
  -- permission_level check, so a future caller reading the function
  -- definition finds the one true check, not a second copy that could drift.
  if pg_get_functiondef('public.sponsor_book_care(uuid,text,uuid)'::regprocedure) like '%permission_level = %'
     or pg_get_functiondef('public.sponsor_request_refill(uuid,uuid)'::regprocedure) like '%permission_level = %'
     or pg_get_functiondef('public.sponsor_pay_booking_order(uuid,text,uuid)'::regprocedure) like '%permission_level = %'
     or pg_get_functiondef('public.sponsor_payable_orders(uuid)'::regprocedure) like '%permission_level = %'
     or pg_get_functiondef('public.redeem_care_voucher(uuid,text,uuid)'::regprocedure) like '%permission_level = %' then
    raise exception 'a caregiver-acting RPC still inlines its own permission_level check instead of calling can_act_for';
  end if;

  if not exists (
    select 1 from pg_policies
     where schemaname = 'public' and tablename in ('care_messages', 'care_message_threads')
       and (coalesce(qual, '') || coalesce(with_check, '')) like '%communicate_with_care_team%'
  ) then
    raise exception 'care_messages/care_message_threads were not narrowed to communicate_with_care_team';
  end if;
end $$;
