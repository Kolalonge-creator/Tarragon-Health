-- Tarragon Health — Consultation System §9.13 "no-show management":
-- "For high-risk patients, a no-show could become a care-management
-- exception." A single missed visit and its patient notification/reschedule
-- offer are the concurrent appointment-engine build's concern (see the note
-- below); a PATTERN of missed visits is a signal worth a coordinator's
-- attention regardless of who records the single event, so it surfaces
-- exactly like every other care gap on the existing patient_care_gaps ->
-- care_outreach_tasks pipeline (20260716150000_care_gap_view.sql /
-- 20260723010019_care_outreach_engine.sql) -- same "no new source of truth"
-- discipline as the awaiting_result gap added 2026-08-03.
--
-- Threshold: 2+ no-shows in the trailing 90 days with nothing completed
-- since the most recent one -- a patient who no-showed twice and then
-- attended is not still a gap; the NOT EXISTS clause is what clears it.
--
-- Reads from public.appointments, not video_consultations. Live-schema
-- inspection (2026-08-28) found a concurrent, uncommitted appointment-engine
-- build already on this project that owns no-show tracking generically
-- across every consultation type (appointments.status = 'no_show', wrapping
-- the channel-specific detail row via appointments.video_consultation_id)
-- -- video_consultations.status = 'no_show' is never actually set by
-- anything live. Building this gap off the real no-show record, not a dead
-- enum value, is what the "no new source of truth" discipline actually
-- requires here.
--
-- View + function both recreated in full (pulled live via pg_get_viewdef /
-- pg_get_functiondef immediately before writing this, not from a prior
-- migration file, precisely because of the concurrent build above), byte-
-- for-byte except for the one new UNION ALL branch and the one new CASE
-- arm -- same discipline the awaiting_result migration itself documented.
