-- Tarragon Health — activate the dormant partner-specialist booking path.
--
-- 20260803142941_self_arranged_specialist_referrals.sql suspended Tarragon-
-- booked specialist referrals because there were zero real partners (all 9
-- specialist_providers rows were [Placeholder], no contact details) — its
-- own comment says "Dormant, not deleted. Contracting a specialist is an
-- is_active flip." But set_referral_specialist_provider was left
-- UNCONDITIONALLY raising ('Specialist booking is not available yet')
-- regardless of whether a real, active, verified provider now exists — so
-- flipping is_active alone would not actually have activated anything. This
-- closes that gap: the RPC now does real work again once a genuine partner
-- is on file, while self_arranged stays the default and unaffected for
-- every referral that never gets an assignment attempt.
--
-- Explicitly NOT touched, guardrail unchanged: no scoring/ranking is added
-- anywhere in this migration. useMatchedSpecialistProviders (app layer)
-- stays a plain filter; staff/patient still pick manually from the filtered
-- list. This migration is entirely about the assignment/booking MECHANICS
-- working again, not about who gets recommended.
--
-- Guardrails re-added in the RPC itself, all missing from the pre-2026-08-03
-- version (which only ever validated fulfilment before its final
-- unconditional raise): the caller must be org staff for the referral's own
-- org, the referral must still be assignable (pending/waitlisted — not
-- already paid/booked/closed), the chosen provider must actually be active,
-- and its specialist_type must match the referral's. The referral's
-- fulfilment flips self_arranged -> partner as part of this same call —
-- that transition IS what "assigning a specialist" now means; a referral
-- that never gets assigned stays self_arranged exactly as before.
