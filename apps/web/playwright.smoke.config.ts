 ...
 *
 * Distinct from the sibling `playwright.config.ts` (`./e2e-browser`, `pnpm
 * test:e2e-browser`), which runs write-capable flows against a fresh LOCAL
 * Supabase stack. This one is a read-only smoke suite against the real
 * production project — see this repo's two separate CI jobs/workflows for
 * why they're kept apart rather than merged into one config: different
 * webServer commands, different target databases, different risk profiles.
 */
