 ...
 *
 * Never throws — the account and session already exist by the time either
 * caller reaches this, so a transport error here (in particular from the
 * `redeem_referral_code` RPC, which can reject outright, not just resolve
 * with `{ ok:false, error }`) must not turn an otherwise-successful signup
 * or confirmation into an error page. Enforced internally (reporting to
 * Sentry) rather than left to each caller to remember to wrap, matching
 * `record-login-device.ts`'s pattern — this already has two callers today
 * and a third self-serve entry point is plausible.
 */
export async function backfillSignupMetadata(
  supabase: SupabaseClient<Database>,
  user: Pick<User, "id" | "user_metadata">,
  /** Distinguishes a Sentry report's origin — "signUp" vs "authCallback". */
  action: string
): Promise<void> {
  try {
    ...
    // Someone who arrived to pay for a relative's care, not to be treated.
    // Only ever narrows what we ask of them; becoming a patient later
    // re-imposes every consent, enforced by enforce_care_purpose_switch.
    ...
    // link, now that a session exists (redeem_referral_code reads
    // auth.uid()). redeem_referral_code enforces its own rules (self-
    // referral, 30-day window, one code per account) and returns
    // { ok:false, error } rather than throwing for those — the outer
    // try/catch below only exists for a genuine transport rejection.
    ...
    Sentry.captureException(err, {
      extra: { action, stage: "metadata_backfill", userId: user.id },
    });
  }
}
