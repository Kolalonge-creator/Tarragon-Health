/**
 * Plain constants shared between server code (embedded-webview.ts, which
 * reads next/headers and is Server-Component-only) and client code
 * (mobile-bridge-gate.tsx, which stamps the cookie below via document.cookie).
 * Kept in their own file with no next/headers import so a client component
 * can import them directly without dragging in a server-only API — that's
 * exactly the bundling error this file exists to avoid.
 */
