 ...
 * order. Backed by lab_order_test_status; a test with no row defaults to
 * "not yet done" without needing one pre-seeded.
 *
 * The upload itself isn't yet scoped to the specific test_code (that needs
 * PatientResultUpload's own test-type field, tracked separately) — this
 * only tracks which test the checklist says is done.
 *
 * ecg_resting routes to EcgReportUpload (its own dedicated table,
 * ecg_report_documents) instead of PatientResultUpload — an ECG has no
 * code-level ambiguity to resolve, there is only ever one per order.
 */
            ...
            {status !== "will_not_do" && (
              <>
                {!isOpen ? (
                  ...
                  <PatientResultUpload labOrderId={labOrderId} label={`Upload: ${testCodeLabel(code)}`} />
                )}
              </>
            )}
          </div>
