...
// `isPlatformModuleEnabled` (only its exported message string is imported
// here) — that helper builds a cookie-based server client
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!topupsModule?.is_enabled) {
    return NextResponse.json({ error: PLATFORM_CREDIT_TOPUPS_DISABLED_MESSAGE }, { status: 403 });
  }
