 ...
 * (AI-005, apps/web/src/app/(dashboard)/patient/result-documents.tsx, buried
 * in Labs & bookings). No new AI system, no new migration -- this is
 * deliberately a thin composition over both already-governed pipelines,
 * placed where a patient actually lands (Overview), because the gap found
 * was discoverability, not missing capability. See the AI Health Assistant
 ...
 */
export function AskTarragonCard({ patientId }: { patientId: string }) {
  ...
  return (
    <Card id="ask-tarragon">
      <CardHeader>
        ...
        <CardDescription>
          Describe what&apos;s worrying you or share a lab result, and get plain-language guidance
          plus a fast way to reach a Tarragon doctor.
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            {lastReply && (
              <div className="space-y-2 rounded-md bg-charcoal-ink/5 dark:bg-night-ink/10 p-3">
                ...
                {handoff.status === "error" && (
                  <p className="text-xs text-red-600 dark:text-red-300">{handoff.error}</p>
                )}
