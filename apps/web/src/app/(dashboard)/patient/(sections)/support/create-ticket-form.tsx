...
/**
 * §24.4's ticket-filing form, narrowed to technical support only (see
 * lib/validation/support-tickets.ts) — every other category from the
 * original spec is handled by the "Need help" card on the Care page
 * (navigation_requests, module 75) instead. If the free text reads like a
 * real emergency, the action never creates a ticket — it raises an
 * emergency_events row instead (§24.7), and this form surfaces that
 * exactly the way DangerSymptomCheck does: invalidate the active-emergency
 * query so the global EmergencyAlert (mounted once in
 * (sections)/layout.tsx) takes over the screen immediately, no separate UI
 * needed here.
 */
    ...
    <form action={formAction} className="space-y-4">
      <div>
        ...
        <input
          ...
          required
          maxLength={200}
          placeholder="Short summary of the app or technical issue"
      ...
      {state && "success" in state && state.success && (
        <p className="text-sm text-brand-green">Sent — our support team will get back to you soon.</p>
      )}
