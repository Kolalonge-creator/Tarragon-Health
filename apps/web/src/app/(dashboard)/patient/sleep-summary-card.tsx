...
import {
  useSleepSummary,
  formatSleepDuration,
  SLEEP_CONSISTENCY_LABEL,
  SLEEP_TREND_LABEL,
} from "@/lib/queries/wearable-sleep";
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        <p className="font-heading text-2xl font-semibold text-charcoal-ink dark:text-night-ink">
          {data.lastNightMinutes !== null ? formatSleepDuration(data.lastNightMinutes) : "—"}
        ...
        {data.averageMinutes !== null && (
          <p className="text-sm text-charcoal-ink/70 dark:text-night-ink/70">
            {formatSleepDuration(data.averageMinutes)} average over the last {data.nightsInWindow}{" "}
            {data.nightsInWindow === 1 ? "night" : "nights"}
          </p>
        )}
        <p className="text-xs text-charcoal-ink/50 dark:text-night-ink/55">{SLEEP_CONSISTENCY_LABEL[data.consistency]}</p>
        <p className="text-xs text-charcoal-ink/50 dark:text-night-ink/55">{SLEEP_TREND_LABEL[data.trend]}</p>
      </CardContent>
    </Card>
  );
}
