    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <SEMANTIC_ICON.booking className="h-5 w-5 text-deep-forest dark:text-brand-green-bright" strokeWidth={2} aria-hidden />
          Your booking requests
        </CardTitle>
      </CardHeader>
      <CardContent>
        {bookingRequests.isLoading && <p className="text-sm text-charcoal-ink/60 dark:text-night-ink/60">Loading…</p>}
        {bookingRequests.isError && (
          <p className="text-sm text-red-600 dark:text-red-300">Could not load your booking requests.</p>
        )}
            ...
            {bookingRequests.data.map((request) => {
              ...
              return (
                <li key={request.id} className="space-y-1 py-3">
                  ...
                  {request.notes && (
                    <p className="text-xs text-charcoal-ink/60 dark:text-night-ink/60">{request.notes}</p>
                  )}
                </li>
              );
            })}
