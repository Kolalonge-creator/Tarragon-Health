...
export default async function OrganisationSettingsPage() {
  ...
  const items = getVisibleItemsForTab("organisation", perms);
  if (items.length === 0) redirect("/admin/settings");

  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-heading text-lg font-semibold text-charcoal-ink">Organisation</h2>
        <p className="text-charcoal-ink/60">
          Company identity and where TarragonHealth currently operates.
        </p>
      </div>
