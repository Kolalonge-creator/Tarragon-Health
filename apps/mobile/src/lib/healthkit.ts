...
export async function configureIOSBackgroundDelivery(): Promise<void> {
  ...
  try {
    ...
    // iOS version quirk, should not block the rest of the app — but still
    // worth recording, since a silently-failed registration here is exactly
    // the kind of "background sync never actually fires" bug that's
    // otherwise invisible until a patient notices stale data.
