...
/**
 * Menstrual cycle prediction engine — verbatim port of
 * apps/web/src/lib/rules/cycle-prediction.ts (duplicated rather than
 * imported since apps/web isn't a shared package the mobile app can pull
 * from, same convention as lib/medications.ts's buildTodaysDoseChecklist and
 * lib/cycle-nudges.ts's port of cycle-nudges.ts — keep the two in sync if
 * the clinical rule changes).
 *
 * Pure, no DB access, no clock reads (today is always passed in) — same
 * discipline as bp-classification, so it's fully unit-testable and safe to
 ...
 *
 * It lives here, next to the logic, rather than inline in a UI component,
 * because it shipped a real bug on web while it was inline: once today
 * passes the point estimate but is still inside the uncertainty window,
 * daysUntilNextPeriod goes negative while isOverdue is still false, and that
 * branch fell through to "Log a period to begin" -- telling somebody with
 * six logged cycles that she had logged nothing.
 */
