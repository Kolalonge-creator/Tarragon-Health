...
import { replaceLabResult, testTypeLabel } from "@/lib/labs";
...
/** Opens the take-anywhere request PDF in Safari's in-app viewer via
 * getLabRequestPrintUrl (GET /api/mobile/lab-order/[orderId]/request) —
 * Safari's own PDF viewer carries Print/Share/Save in its toolbar, so
 * there's nothing further to build here for "print". */
function PrintRequestButton({ orderId }: { orderId: string }) {
  ...
  async function open() {
    setLoading(true);
    setError(null);
    const result = await getLabRequestPrintUrl(orderId);
    setLoading(false);
    if (!result.ok) {
      setError(result.error);
      return;
  ...
  return (
    <View style={{ gap: 4 }}>
      <SecondaryButton title={loading ? "Opening…" : "Print the request"} onPress={open} disabled={loading} />
      {error ? <ErrorText>{error}</ErrorText> : null}
    </View>
  );
}
 ...
 * breakdown, preparation instructions, and (for a self-bookable bundle) a
 * "Get & print" action, mirroring the web catalogue's <details> dropdown
 * and annual-health-check-booking.tsx's "Get & print" button. Local
 * open/closed state only; nothing here is persisted or shared across
 * bundles. A bundle with selfBookable=false (a component of a bigger
 * panel, e.g. single_hba1c) still expands to show its detail, it just gets
 * no order action — same distinction the web page draws. */
function LabCatalogueRow({
  bundle,
  patientId,
  organisationId,
  onOrdered,
}: {
  bundle: LabCatalogueItem;
  ...
  async function getAndPrint() {
    if (!patientId || !organisationId) {
      setError("Could not set that up just now. Please try again.");
      return;
    }
    setOrdering(true);
    setError(null);
    const created = await createLabOrder(organisationId, patientId, bundle.id);
    if (!created.ok) {
      setOrdering(false);
      setError("Could not set that up just now. Please try again.");
      return;
    ...
    onOrdered();
    const printUrl = await getLabRequestPrintUrl(created.data.id);
    setOrdering(false);
    if (printUrl.ok) {
      await WebBrowser.openBrowserAsync(printUrl.data);
    }
  }
      ...
      {open ? (
        <View style={{ gap: 8, paddingTop: 2 }}>
          ...
          {bundle.selfBookable ? (
            <View style={{ gap: 4 }}>
              <SecondaryButton
                title={ordering ? "Getting your form ready…" : `Get & print ${bundle.name}`}
                onPress={getAndPrint}
                disabled={ordering}
              />
              {error ? <ErrorText>{error}</ErrorText> : null}
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { data: profile } = await supabase
        ...
        .eq("id", subjectId)
        .single();
      if (!cancelled) setOrganisationId(profile?.organisation_id ?? null);
            ...
            {orders.data.map((order) => {
              ...
              return (
                <Card key={order.id} style={{ gap: 6 }}>
                  ...
                  {awaiting ? (
                    <>
                      <PrintRequestButton orderId={order.id} />
                      {order.testCodes.length > 1 ? (
                        <LabOrderTestChecklist labOrderId={order.id} testCodes={order.testCodes} />
                      ) : (
                        <MutedText>
                          You pay the lab directly, at whatever they charge. Use &quot;Upload a result&quot; above
                          once you have it.
                ...
                <GroupedList>
                  {bundles.map((bundle) => (
                    <LabCatalogueRow
                      ...
                      onOrdered={() => patientId && load(patientId)}
                    />
                  ))}
                </GroupedList>
              </View>
